...
describe("it is case insensitive", () => {
  test("when passed x it returns 10", () => {
    expect(romanNumeralConverter("x")).toBe(10);
  });
  test("when passed xxiv it returns 24", () => {
    expect(romanNumeralConverter("xxiv")).toBe(24);
  });
  test('when passed l it returns 50', () => {
    expect(romanNumeralConverter('l')).toBe(50)  
  })
  ...
  test('when passed mdxxiii it returns 10', () => {
    expect(romanNumeralConverter('mdxxiii')).toBe(1523)  
  })
});

describe("edge cases", () => {
  test("when passed a string that does not contain only roman numerals it throws an error", () => {
    const errorMessage = "Please feed me Roman Numerals only";
    expect(() => romanNumeralConverter("not a roman numeral")).toThrow(
      errorMessage
    );
  });
});
